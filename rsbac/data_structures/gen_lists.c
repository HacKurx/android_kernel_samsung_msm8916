...
static void rcu_free(struct rsbac_list_reg_item_t * list, void * mem)
{
	...
	if (unlikely(!create_rcu_free(list))) {
...
static void rcu_free_lol(struct rsbac_list_lol_reg_item_t * list, void * mem)
{
	...
	if (unlikely(!create_rcu_free_lol(list))) {
			...
			rsbac_list_handle_t to_handle)
{
	...
	if (unlikely(!from_handle || !to_handle))
		return -RSBAC_EINVALIDLIST;

	from_list = (struct rsbac_list_reg_item_t *) from_handle;
	if (unlikely(from_list->self != from_list))
		return -RSBAC_EINVALIDLIST;
	to_list = (struct rsbac_list_reg_item_t *) to_handle;
	if (unlikely(to_list->self != to_list))
		return -RSBAC_EINVALIDLIST;
	if(unlikely((from_list->info.desc_size != to_list->info.desc_size)
...
*/
	...
	if(unlikely(ta_number))
		for (i=0; i<to_list->nr_hashes; i++)
			if(unlikely(to_list->hashed[i].ta_copied != ta_number)) {
				err = -RSBAC_EBUSY;
				goto out_unlock;
			}
#endif
			...
			rsbac_list_handle_t to_handle)
{
	...
	while (ta_committing)
		interruptible_sleep_on(&ta_wait);
	if (unlikely(ta_number && !rsbac_ta_list_exist(0, ta_handle, &ta_number)))
		return -RSBAC_EINVALIDTRANSACTION;
#endif
	...
	for (i=0; i<nr_from_hashes; i++) {
		...
		if (unlikely(ta_number)) {
			...
			while(item_p) {
				...
				while (subitem_p) {
					if (to_list->subslab)
						new_subitem_p = rsbac_smalloc(to_list->subslab);
					else
						new_subitem_p = rsbac_kmalloc(subitem_size);
					if (!new_subitem_p) {
						err = -RSBAC_ENOMEM;
...
#ifdef CONFIG_RSBAC_DEBUG
				if(rsbac_debug_auto)
					rsbac_printk(KERN_DEBUG "rsbac_list_auto_rehash(): changing list %s hash size on device %02u:%02u from %u to %u\n",
...
#ifdef CONFIG_RSBAC_DEBUG
				if(rsbac_debug_auto)
					rsbac_printk(KERN_DEBUG "rsbac_list_auto_rehash(): changing list of lists %s hash size on device %02u:%02u from %u to %u\n",
...
#ifdef CONFIG_RSBAC_DEBUG
	if(rsbac_debug_auto && (nr_rehashed > 0))
