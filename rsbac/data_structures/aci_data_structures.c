...
#endif
#if defined(CONFIG_RSBAC_UDF)
static u_int udf_nr_fd_hashes = RSBAC_LIST_MIN_MAX_HASHES;

#if defined(CONFIG_RSBAC_UDF_CACHE)
static u_int udf_checked_nr_fd_hashes = RSBAC_LIST_MIN_MAX_HASHES;
#endif
#endif
				  ...
				  rsbac_boolean_t create_dir, kdev_t kdev)
{
	...
	if (!vfsmount_p) {
		...
		if (!vfsmount_p) {
			rsbac_printk(KERN_WARNING "lookup_aci_path_dentry(): unknown device %02u:%02u\n",
			     ...
			     kdev_t kdev)
{
...
#if defined(CONFIG_RSBAC_UDF)
	{
		...
		info_p->max_age = 0;
		udf_nr_fd_hashes = RSBAC_UDF_NR_FD_LISTS;
		tmperr = rsbac_list_register_hashed(RSBAC_LIST_VERSION,
...
#if defined(CONFIG_RSBAC_UDF_CACHE)
	{
		...
		info_p->max_age = 0;
		udf_checked_nr_fd_hashes = RSBAC_UDF_CHECKED_NR_FD_LISTS;
		tmperr = rsbac_list_register_hashed(RSBAC_LIST_VERSION,
					     ...
					     RSBAC_UDF_CHECKED_FD_NAME, kdev,
					     udf_checked_nr_fd_hashes,
					     rsbac_list_hash_fd,
					     NULL);
		if (tmperr) {
			char *tmp;
...
int rsbac_write_open(char *name, struct file **file_pi, kdev_t kdev)
{
	...
	/* alloc_file() will call mnt_want_write */
...
static int pax_proc_open(struct inode *inode, struct file *file)
{
...
#endif
{
	...
	if (err)
		...
		rsbac_list_lol_max_items(fd_cache_handle[SW_DAZ],
			...
			A_none);
#endif
#if defined(CONFIG_RSBAC_UDF) && !defined(CONFIG_RSBAC_UDF_CACHE)
	sprintf(tmp, "%sUDF", RSBAC_FD_CACHE_NAME);
	err = rsbac_list_lol_register_hashed(RSBAC_LIST_VERSION,
				  &fd_cache_handle[SW_UDF], list_info_p,
				  RSBAC_LIST_DEF_DATA | RSBAC_LIST_OWN_SLAB | \
				    RSBAC_LIST_AUTO_HASH_RESIZE | \
				    RSBAC_LIST_NO_MAX_WARN,
				  NULL,
				  ...
				  NULL);
	if (err)
		registration_error(err, "FD Cache UDF");
	else
		rsbac_list_lol_max_items(fd_cache_handle[SW_UDF],
			RSBAC_FD_CACHE_KEY,
			CONFIG_RSBAC_FD_CACHE_MAX_ITEMS,
			A_none);
#endif
...
/* call other umount functions */
	...
	while (!RSBAC_IS_AUTO_DEV(umount_device_in_progress)) {
		...
		init_timer_on_stack(&auto_timer);
		...
		case SW_UDF:
			{
...
#endif
				{
					...
					case A_udf_checker:
						...
						break;
					case A_udf_do_check:
						value->udf_do_check = aci.udf_do_check;
						if(   (value->udf_do_check == UDF_inherit)
							&& inherit) {
							enum rsbac_target_t       parent_target;
							union rsbac_target_id_t   parent_tid;
	...
	case SW_PAX:
		{
			...
			switch (attr) {
			case A_pax_flags:
				task_p = get_pid_task(tid_p->process, PIDTYPE_PID);
				if (task_p) {
					if (pid_alive(task_p)) {
	...
	case SW_UDF:
		{
			...
			if (attr == A_udf_checked) {
				err =
				    rsbac_list_add_ttl(device_p->handles.udfc,
						       rsbac_udf_ttl,
						       &tid_p->file.inode,
						       &value_p->
						       udf_checked);
