...
int rsbac_um_hash(char *pass, __u32 salt)
{
	...
	if (!tfm) {
		rsbac_printk(KERN_WARNING "pid %u(%s): rsbac_um_hash(): Could not allocate tfm for SHA1!\n",
	...
	if(err) {
		rsbac_printk(KERN_WARNING "pid %u(%s): rsbac_um_hash(): crypto_hash_init() failed with error %u!\n",
			current->pid, current->comm, err);
		goto out;
	}
	err = crypto_hash_update(&hd, sg, sg[0].length);
	if(err) {
		rsbac_printk(KERN_WARNING "pid %u(%s): rsbac_um_hash(): crypto_hash_update() failed with error %u!\n",
			current->pid, current->comm, err);
		goto out;
	}
	err = crypto_hash_final(&hd, pass);
	if(err) {
		rsbac_printk(KERN_WARNING "pid %u(%s): rsbac_um_hash(): crypto_hash_final() failed with error %u!\n",
			current->pid, current->comm, err);
		goto out;
	}
		      ...
		      char *pass, rsbac_time_t ttl)
{
	...
	if (RSBAC_UID_SET(user))
		rsbac_pr_debug(aef_um, "pid %u(%s): adding user %u/%u\n",
			current->pid, current->comm,
			RSBAC_UID_SET(user), RSBAC_UID_NUM(user));
	else
#endif
		rsbac_pr_debug(aef_um, "pid %u(%s): adding user %u\n",
		       ...
		       char *pass, rsbac_time_t ttl)
{
	...
	if (RSBAC_GID_SET(*group_p))
		rsbac_pr_debug(aef_um, "pid %u(%s): adding group %u/%u\n",
			current->pid, current->comm,
			RSBAC_GID_SET(*group_p), RSBAC_GID_NUM(*group_p));
	else
		rsbac_pr_debug(aef_um, "pid %u(%s): adding group %u\n",
		      ...
		      union rsbac_um_mod_data_t *data_p)
{
	...
	rsbac_pr_debug(aef_um, "pid %u(%s): modifying user %u\n",
...
int rsbac_um_check_pass(rsbac_uid_t uid, char *pass)
{
	...
	if(RSBAC_UID_SET(uid))
		rsbac_pr_debug(aef_um, "pid %u(%s): checking password for user %u/%u\n",
			current->pid, current->comm, RSBAC_UID_SET(uid), RSBAC_UID_NUM(uid));
	else
#endif
		rsbac_pr_debug(aef_um, "pid %u(%s): checking password for user %u\n",
		...
		if (count > 0) {
			...
			for (i=0; i<count ;i++) {
				...
				if (!memcmp
					...
					rsbac_pr_debug(aef_um, "pid %u(%s): one-time password %u for user %u/%u matched, removing\n",
...
int rsbac_um_add_onetime(rsbac_uid_t uid, char *pass, rsbac_time_t ttl)
{
	...
	if (RSBAC_UID_SET(uid))
		rsbac_pr_debug(aef_um, "pid %u(%s): add one-time password for user %u/%u with ttl %lu\n",
			current->pid, current->comm, RSBAC_UID_SET(uid), RSBAC_UID_NUM(uid), ttl);
	else
		rsbac_pr_debug(aef_um, "pid %u(%s): add one-time password for user %u with ttl %lu\n",
...
#ifdef CONFIG_RSBAC_UM_VIRTUAL
	if(RSBAC_UID_SET(uid))
		rsbac_pr_debug(aef_um, "pid %u(%s): setting password for user %u/%u\n",
			current->pid, current->comm, RSBAC_UID_SET(uid), RSBAC_UID_NUM(uid));
	else
#endif
		rsbac_pr_debug(aef_um, "pid %u(%s): setting password for user %u\n",
			current->pid, current->comm, RSBAC_UID_NUM(uid));
	if (pass) {
#ifdef CONFIG_RSBAC_UM_PWHISTORY
		__u32 max_index = 0;
...
#ifdef CONFIG_RSBAC_UM_VIRTUAL
	if(RSBAC_GID_SET(gid))
		rsbac_pr_debug(aef_um, "pid %u(%s): setting password for group %u/%u\n",
			current->pid, current->comm, RSBAC_GID_SET(gid), RSBAC_GID_NUM(gid));
	else
#endif
		rsbac_pr_debug(aef_um, "pid %u(%s): setting password for group %u\n",
