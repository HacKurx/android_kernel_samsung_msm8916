...
/* i could have done it in few lines of code, but that's way it is MUCH faster and sendfile is mostly beeing used with network sockets */
	if (S_ISSOCK(in.file->f_dentry->d_inode->i_mode)) {
		sock1 = SOCKET_I(in.file->f_dentry->d_inode);
...
#ifdef CONFIG_RSBAC_RW
	if (S_ISSOCK(out.file->f_dentry->d_inode->i_mode)) {
		sock2 = SOCKET_I(out.file->f_dentry->d_inode);
